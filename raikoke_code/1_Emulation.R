...
# Load train/val inds
train_inds <- readRDS("data/train_inds.rds")
...
# dir.create('data/EmT1') # if this emulator hasn't been created before, create a directory to store it in
# EmT1 <- BuildNewEmulators(tData = tDataT1, HowManyEmulators = 1, meanFun="fitted", kernel = c('Matern52'), Choices = list(PriorT1))
# save_ExUQmogp(EmT1, file = 'data/EmT1/train')
...
# }
# SaveMulti(EmT5_MET, file = 'data/EmT5_MET/train')
...
for (i in c(1:5,7:8)){
  ...
  save_ExUQmogp(EmRegion, file = paste0('data/EmRegion/region', i))
}
...
save_ExUQmogp(EmRegion, file = paste0('data/EmRegion/region', 6))

# Fitting MET emulators
# These are all fast (O(seconds), as at most ~45 training points)
dir.create('data/EmRegion_MET')
twd <- getwd()
setwd('data/EmRegion_MET')
for (i in c(1:5,7:8)){
  EmRegion_MET <- NULL
  for (m in 0:17){
